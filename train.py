...
def train_one_epoch(
    ...
    for step, samples in enumerate(dataloader):
        ...
        if len(detr_loss_dict) == 0:
            detr_loss_dict = {"loss_ce": torch.tensor(0.0, device=device)}
        ...
        if not only_detr:
            ...
            model_without_ddp.id_decoder.shuffle()
            ...
            # ============================
            # Representation stage (NEW)
            # ============================
            rep_losses = {}

            if hasattr(model, "identity_representation") and model.identity_representation is not None:
                rep_out = model(seq_info=seq_info, part="representation")

                # 🔒 Contract enforcement
                assert "seq_info" in rep_out, "representation must return seq_info"
                assert "losses" in rep_out, "representation must return losses dict"

                seq_info = rep_out["seq_info"]
                rep_losses = rep_out["losses"]
            # ============================
            # Representation stage (END)
            # ============================

            if accelerator.is_main_process and step == 0 and rep_losses:
                logger.info(
                    f"[Epoch {epoch}] Identity representation enabled. "
                    f"Active losses: {list(rep_losses.keys())}"
                )
            ...
            id_loss = id_criterion(id_logits, id_gts, id_masks)
        ...
        with accelerator.autocast():
            ...
            # RF-DETR: if DETR is frozen, detr_loss becomes empty → int(0)
            if not torch.is_tensor(detr_loss):
                detr_loss = torch.tensor(0.0, device=device)


            loss = detr_loss + (id_loss if id_loss is not None else 0) * id_criterion.weight
            # ============================
            # Representation losses (NEW)
            # ============================
            for loss_name, loss_value in rep_losses.items():
                loss = loss + loss_value
                metrics.update(name=loss_name, value=loss_value.item())
            ...
            for k, v in detr_loss_dict.items():
                if torch.is_tensor(v):
                    ...
                    metrics.update(name=k, value=float(v))
