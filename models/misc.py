...
def load_detr_pretrain(model: nn.Module, pretrain_path: str, num_classes: int | None, default_class_idx: int | None = None):
    ...
    # ============================================================
    # 1) AUTO-SKIP FOR RF-DETR (LWDETR)
    # ============================================================
    if hasattr(model, "detr") and model.detr.__class__.__name__.lower().startswith("lwdetr"):
        print("[INFO] RF-DETR detected → skipping load_detr_pretrain().")
        return

    # Sometimes MOTIP stores DETR directly in model (not under model.detr)
    if model.__class__.__name__.lower().startswith("lwdetr"):
        print("[INFO] RF-DETR detected → skipping load_detr_pretrain().")
        return    

    # ============================================================
    # 2) SKIP IF PATH IS NONE / EMPTY / INVALID
    # ============================================================
    if pretrain_path is None or pretrain_path == "" or not isinstance(pretrain_path, str):
        print(f"[INFO] Skipping DETR pretrain load: invalid path '{pretrain_path}'")
        return

    if not os.path.exists(pretrain_path):
        print(f"[INFO] Pretrain path does not exist → skipping: '{pretrain_path}'")
        return

    # ============================================================
    # 3) LOAD DEFORMABLE-DETR CHECKPOINT (OLD MOTIP ONLY)
    # ============================================================
    print(f"[INFO] Loading deformable-DETR pretrained weights from: {pretrain_path}")
    ...
    pretrain_state_dict = pretrain_model["model"] if "model" in pretrain_model else pretrain_model

    detr_state_dict = {}
    model_state_dict = model.state_dict()

    # Prefix all keys with "detr."
    for k, v in pretrain_state_dict.items():
        detr_state_dict["detr." + k] = v

    # Process classifier, label encoder, etc.
    for k, v in detr_state_dict.items():

        # classifier special handling
        if "class_embed" in k:
            if num_classes is None:
                num_classes = len(v)
            ...
            if len(v) == 91:
                ...
                if num_classes == 1:
                    if default_class_idx is None:
                        detr_state_dict[k] = v[1:2]  # person
                    else:
                        detr_state_dict[k] = v[default_class_idx:default_class_idx+1]
                else:
                    raise NotImplementedError(
                        f"Cannot adapt COCO classifier for num_classes={num_classes}"
                    )

        # label encoder special handling
        if "label_enc" in k:
            if len(v) != len(model_state_dict[k]):
                if len(model_state_dict[k]) == 2:
                    detr_state_dict[k] = torch.cat((v[1:2], v[91:92]), dim=0)
                else:
                    raise NotImplementedError(
                        "Cannot adapt label_enc for given num_classes"
                    )

    # Inject the pre-trained parameters
    for k, v in detr_state_dict.items():
        assert k in model_state_dict, f"DETR key missing in model: {k}"
        model_state_dict[k] = v

    model.load_state_dict(model_state_dict, strict=True)
    print("[INFO] Pretrained Deformable-DETR weights loaded successfully.")
