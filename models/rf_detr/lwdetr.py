...
class DummyCriterion(nn.Module):
    def __init__(self, matcher):
        ...
        self.weight_dict = {}   # empty because no DETR losses
        self.losses = []        # for compatibility
    ...
    def forward(self, outputs, targets, batch_len=None, **kwargs):
        ...
        return {}, indices        # <-- IMPORTANT: return NO losses
...
def build_motip_criterion(args):
    device = torch.device(args.device)
    matcher = build_matcher(args)  # IMPORTANT
    criterion = DummyCriterion(matcher=matcher)
    criterion.to(device)
